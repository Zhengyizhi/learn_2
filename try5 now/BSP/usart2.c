...
void DMA1_Stream5_IRQHandler(void)
{
	...
	if(DMA_GetITStatus(DMA1_Stream5, DMA_IT_TCIF5))
	{
		...
		{
			if(RC_Ctl.rc.sw <= 500)//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!摩擦轮一直开着进入键盘模式
			{
				KeyFlag.Keystart_Flag = stillwork;
				AllFlag.SteerEngine_level = none_;
				AllFlag.Fiction_level = none_;
				AllFlag.Dial_level_mech = none_;
				AllFlag.Dial_level_gyro = none_;
				AllFlag.Dial_level_smallgyro = none_;//初始化
				KeyFlag.fictioninit = open;
			}//瞬间
			else if(RC_Ctl.rc.sw >= 1500)
			{
				KeyFlag.Keystart_Flag = unwork;
			}
			if(KeyFlag.Keystart_Flag == unwork)
			{
//////////////////////////////////////////////////////////////////////////////			
					if(RC_Ctl.rc.s2 == 3)//机械模式
					{
						AllFlag.goruning = mech_condition;//运行
						if(RC_Ctl.rc.s1 == 3)
						{
							AllFlag.SteerEngine_level = level1;//舵机
						}
						if(RC_Ctl.rc.s1 == 2)
							AllFlag.SteerEngine_level = none_;
						if(RC_Ctl.rc.s1 == 1 && AllFlag.SteerEngine_level == level1)
						{
							AllFlag.SteerEngine_level = level2;//舵机工作
						}
					}
					else if(RC_Ctl.rc.s2 == 2)
					{
						AllFlag.goruning = gyro_condition;
						if(RC_Ctl.rc.s1 == 1)
						{
							AllFlag.Fiction_level = level1;
						}
						if(RC_Ctl.rc.s1 == 3 && AllFlag.Fiction_level == level1)
						{
							AllFlag.Fiction_level = level2;//摩擦轮工作
						}
						if(RC_Ctl.rc.s1 == 2)
							AllFlag.Fiction_level = none_;
					}
					...
					{
						AllFlag.goruning = smallgyro_condition;
					}
			
					
					if(AllFlag.Fiction_Flag == stillwork)//打开状态%2
					{
						if(AllFlag.goruning == mech_condition)
						{
							if(RC_Ctl.rc.s1 == 3)
							{
								AllFlag.Dial_level_mech = level1;
							}
							if(RC_Ctl.rc.s1 == 2 && AllFlag.Dial_level_mech == level1)
							{
								AllFlag.Dial_level_mech = level2;
							}
							if(RC_Ctl.rc.s1 == 1)
								AllFlag.Dial_level_mech = none_;
						}
						else if(AllFlag.goruning == gyro_condition)
						{
							if(RC_Ctl.rc.s1 == 3)
							{
								AllFlag.Dial_level_gyro = level1;
							}
							if(RC_Ctl.rc.s1 == 2 && AllFlag.Dial_level_gyro == level1)
							{
								AllFlag.Dial_level_gyro = level2;
							}
							if(RC_Ctl.rc.s1 == 1)
								AllFlag.Dial_level_gyro = none_;
						}
						else if(AllFlag.goruning == smallgyro_condition)
						{
							if(RC_Ctl.rc.s1 == 3)
								AllFlag.Dial_level_smallgyro = level1;
							else if(RC_Ctl.rc.s1 == 2)
								AllFlag.Dial_level_smallgyro = level2;
							else if(RC_Ctl.rc.s1 == 1)
								AllFlag.Dial_level_smallgyro = none_;
						}
					}
					else if(AllFlag.Fiction_Flag == unwork)//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					{
						AllFlag.Dial_level_gyro = none_;
						AllFlag.Dial_level_mech = none_;
					}
